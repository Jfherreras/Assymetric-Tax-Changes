...
********************************************************************************
* This do-file produces: Panel Regression on Inequality
********************************************************************************
...
* Col 1: simple no FE
eststo: reghdfe gini corporateincometaxrate, noabs nocons
	estadd local country $\times$ 
	estadd local year $\times$ 
* Col 2: controlling by profits and wages no FE
eststo: reghdfe gini corporateincometaxrate profit_rate1 wages_lc_c unemploymentrate, noabs nocons
	estadd local country $\times$ 
	estadd local year $\times$ 
* Col 3: simple country FE
eststo: reghdfe gini corporateincometaxrate, abs(country) nocons
	estadd local country $\checkmark$ 
	estadd local year $\times$ 	
* Col 4: controlling by profits and wages
eststo: reghdfe gini corporateincometaxrate profit_rate1 wages_lc_c unemploymentrate, abs(country) nocons
	estadd local country $\checkmark$ 
	estadd local year $\times$ 
* Col 5: simple time FE
eststo: reghdfe gini corporateincometaxrate, abs(year) nocons
	estadd local country $\times$ 
	estadd local year $\checkmark$ 	
* Col 6: controlling by profits and wages time FE
eststo: reghdfe gini corporateincometaxrate profit_rate1 wages_lc_c unemploymentrate, abs(year) nocons
	estadd local country $\times$ 
	estadd local year $\checkmark$	
* Col 7: simple country year FE
eststo: reghdfe gini corporateincometaxrate, abs(country year) nocons
	estadd local country $\checkmark$ 
	estadd local year $\checkmark$	
* Col 8: controlling by profits and wages country year FE
eststo: reghdfe gini corporateincometaxrate profit_rate1 wages_lc_c unemploymentrate, abs(country year) nocons
	estadd local country $\checkmark$ 
	estadd local year $\checkmark$	
	
* Output table
esttab using "$tabfolder/Table_3_Panel_Regression_Ineq.tex", replace f se  ///
...
posthead("& \multicolumn{8}{c}{Inequality} \\ \hline &  &  &  &  &  &  &  &  &  \\") ///
